    ...
    <?php  echo '<h4 class="highlight mb20">미디어 통계</h4>'; ?>
        <div class="box-table-header">
            <ul class="nav nav-pills">
                    ...
                    <li role="presentation" class="active"><a href="<?php echo site_url($this->pagedir . '/graph/selfcert_ad?'.$this->param->output()); ?>">기간별 그래프</a></li>
...
function drawChart() {
    ...
    if($this->input->get('datetype') === 'domain'){
     ...
     <?php } else {?>
        ...
        data.addColumn('number', '본인인증 클릭 수');
        data.addColumn('number', '미디어 노출 수');
        data.addRows([
            <?php
            if (element('list', $view)) {
                foreach (element('list', $view) as $key => $result) {
