        ...
        <table class="table table-hover" >
            <thead>
                <tr>
                    ...
                    <th>번호</a></th>
                    <?php 
                    if(element('post_id',element('post',$view)) === "120"){ 
                        echo '<th>보호자이름</th>';
                        echo '<th>자녀이름</th>';
                    } else {
                        echo '<th>이름</th>';
                    ...
                    <th>사유</th>
                    <?php 
                    if (element('tenping_extra_vars', element('data', element('list', $view)))){
                        echo '<th>질문1</th>
                    <th>질문2</th>
                    <th>질문3</th>
                    ';
            ...
            ?>
                 <tr>
                    ...
                    <?php 
                    if(element('post_id',element('post',$view)) === "120"){                         
                        echo '<td>'.element('mlh_name_sub', $result).'</td>';
                    } 
                    ?>                    
    ...
    <div class="border_button">
        ...
        <div class="box-info">
            <div class="btn-group pull-right" role="group" aria-label="...">
                ...
                <button type="button" class="btn btn-outline btn-danger btn-sm mr10" id="export_to_tenping">체크 항목 API 연동</button> 
                <?php } ?>
...
$(document).on('click', '#export_to_excel', function(){
    exporturl = '<?php echo site_url('board_post/excel_viashare_history/'.element('brd_key',element('board',$view)).'/'.element('post_id',element('post',$view)) . '?' . $this->input->server('QUERY_STRING', null, '')); ?>';
        ...
        if (confirm("한번 전송한 자료는 수정 할 수 없습니다.\n\n전송하시겠습니까?")) {
            ...
            if (element('list', element('data', element('api_list', $view)))) {
                foreach (element('list', element('data', element('api_list', $view))) as $result) {
                    $status='';
                    $via_token='';
                    if(element('mlh_status', $result) ==="2") $status='R';
                    elseif(element('mlh_status', $result) ==="1") $status='C';

                    if(empty(element('via_token', $result))) $via_token=date('Ymd');
                    else $via_token = element('via_token', $result);
                    
            ?>
            if($("#chk_mlh_id_<?php echo element('mlh_id',$result)?>").prop("checked")){
                 act_cpaProc('<?php echo element('mlh_id',$result)?>','<?php echo $via_token?>','<?php echo $status?>');
             }
            <?php 
                } 
            } 
...
function act_cpaProc(mlh_id,via_token,status) {
    var href;
    
    href = cb_url + '/postact/viashere_cpa_curl/'+mlh_id+'/'+via_token+'/'+status;
    $.ajax({
        async : false,
        url : href,
        type : 'get',
        ...
        complete : function (response) {
            if (response.responseJSON.error) {
                ...
                $.ajax({
                     async : false,
                     url : cb_url + '/media/act_viaProc/' +mlh_id+'/'+response.responseJSON.ResultCode+'/'+response.responseJSON.Message,
                     type : 'get',
                     dataType : 'json',
