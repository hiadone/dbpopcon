...
<div class="board mt40">
    ...
    <div class="border_button">
        <div class="pull-left mr10">
            ...
            <a href="<?php echo element('list_url', element('list', $view)); ?>?linkasiastatus=1" class="btn btn-success btn-sm">유효목록</a>
            <a href="<?php echo element('list_url', element('list', $view)); ?>?linkasiastatus=2" class="btn btn-danger btn-sm">무효목록</a>
        </div>
        <div class="box-info">
            <div class="btn-group pull-right" role="group" aria-label="...">
                <?php if (element('is_admin', $view) && $this->member->item('mem_userid') !=='admin_asia') { ?>
                ...
                <button type="button" class="btn btn-outline btn-danger btn-sm mr10" id="export_to_linkasia">체크 항목 API 연동</button> 
                <?php } ?>
...
$(document).on('click', '#export_to_excel', function(){
    exporturl = '<?php echo site_url('board_post/excel_linkasia_history/'.element('brd_key',element('board',$view)).'/'.element('post_id',element('post',$view)) . '?' . $this->input->server('QUERY_STRING', null, '')); ?>';
    document.location.href = exporturl;
});

$(document).on('click', '#export_to_linkasia', function() {
        if (confirm("한번 전송한 자료는 수정 할 수 없습니다.\n\n전송하시겠습니까?")) {
            ...
            if (element('list', element('data', element('api_list', $view)))) {
                foreach (element('list', element('data', element('api_list', $view))) as $result) {
                    $status='';
                    $jid='';
                    if(element('mlh_status', $result) ==="2") $status='declined';
                    elseif(element('mlh_status', $result) ==="1") $status='confirmed';
...
function act_cpaProc(mlh_id,jid,status,reason) {
    ...
    href = cb_url + '/postact/linkasia_cpa_curl/'+jid+'/'+status;
    $.ajax({
        async : false,
        url : href,
        type : 'get',
