...
<div class="board mt40">
    ...
    <div class="row">전체 : <?php echo element('total_rows', element('data', element('list', $view))); ?>건</div>
        <table class="table table-hover" >
            ...
            ?>
                 <tr>
                    <th scope="row"><input type="checkbox" name="chk_post_id[]" value="<?php echo element('mlh_id', $result); ?>"  /></th>
                    ...
                    <?php
                        ...
                        if(element('mlh_api_flag2', $result) === "1") {
                            if(element('mlh_rst2', $result) === "S") 
                                echo "<td>성공</td>";
                            else 
                                echo "<td>".element('mlh_msg2', $result)."</td>";
                        }else echo "<td>-</td>";
    ...
    <div class="border_button">
        <div class="pull-left mr10">
            ...
            <a href="<?php echo element('list_url', element('list', $view)); ?>?linkminestatus=1" class="btn btn-success btn-sm">유효목록</a>
            <a href="<?php echo element('list_url', element('list', $view)); ?>?linkminestatus=2" class="btn btn-danger btn-sm">무효목록</a>
        </div>
        <div class="box-info">
            <div class="btn-group pull-right" role="group" aria-label="...">
                ...
                <button type="button" class="btn btn-outline btn-danger btn-sm mr10" id="export_to_linkmine">링크마인 API 연동</button> 
                <?php } ?>
...
$(document).on('click', '#export_to_excel', function(){
    exporturl = '<?php echo site_url('board_post/excel_linkmine_history/'.element('brd_key',element('board',$view)).'/'.element('post_id',element('post',$view)) . '?' . $this->input->server('QUERY_STRING', null, '')); ?>';
    document.location.href = exporturl;
});

$(document).on('click', '#export_to_linkmine', function() {
        if (confirm("한번 전송한 자료는 수정 할 수 없습니다.\n\n전송하시겠습니까?")) {
            ...
            if (element('list', element('data', element('api_list', $view)))) {
                foreach (element('list', element('data', element('api_list', $view))) as $result) {
                    if(element('mlh_status', $result) ==='2') continue;
            ?>
                act_cpaProc('<?php echo element('mlh_id',$result)?>','<?php echo element('adf_key',$result)?>','<?php echo element('adv_key',$view)?>');
            <?php 
                } 
            } 
            ?>
            alert('링크마인 API 연동이 완료 되었습니다.');
            return false;
        } else {
            return false;
        }
...
function act_cpaProc(mlh_id,adf_key,adv_key) {
    var href;

    href = 'http://api.linkmine.co.kr/callback/act_cpa.proc?adf_key='+adf_key+'&adv_key='+adv_key;
    ...
    $.ajax({
        ...
        complete : function (response) {
            ...
            $.ajax({
                 async : false,
                 url : cb_url + '/media/act_cpaProc/' +mlh_id+'/'+data.rst+'/'+encodeURI(data.msg),
                 type : 'get',
                 dataType : 'json',
