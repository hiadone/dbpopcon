        ...
        <table class="table table-hover" >
            <thead>
                <tr>
                    ...
                    <th>신청일시</th>
                    <?php echo element('post_id',element('post',$view)) ==="484" ? '<th>랜탈문의</th>' : '';?>

                    <th class="per20"><?php echo element('post_id',element('post',$view)) ==="484" ? '통화가능시간' : '문의사항';?></th>
                    <th>상태</th>
                    <th>사유</th>
            ...
            ?>
                 <tr>
                    ...
                    <?php echo element('post_id',element('post',$view)) ==="484" ? '<td>'.element('mlh_val1', $result).'</td>' : '';?>
        ...
        if (confirm("한번 전송한 자료는 수정 할 수 없습니다.\n\n전송하시겠습니까?")) {
            ...
            if (element('list', element('data', element('api_list', $view)))) {
                foreach (element('list', element('data', element('api_list', $view))) as $result) {
                    $status='';
                    $jid='';
                    if(element('mlh_status', $result) ==="2") $status='db_refuse';
                    elseif(element('mlh_status', $result) ==="1") $status='db_ok';
...
function act_cpaProc(mlh_id,jid,status,reason) {
    ...
    href = cb_url + '/postact/adpot_cpa_curl/'+jid+'/'+status+'/'+reason;
    $.ajax({
        async : false,
        url : href,
        type : 'get',
        ...
        complete : function (response) {
            if (response.responseJSON.error) {
                ...
                $.ajax({
                     async : false,
                     url : cb_url + '/Media_multiple/act_tpProc/' +mlh_id+'/'+response.responseJSON.ResultCode+'/'+response.responseJSON.Message,
                     type : 'get',
                     dataType : 'json',
