...
<div class="board">
    ...
    <?php if (element('link_count', $view) > 0 OR element('file_download_count', $view) > 0) { ?>
        <ul class="list-group">
            ...
            foreach (element('link', $view) as $key => $value) {
                if(empty(element('campaign_multi', $view))){
                    
                    ?>
                        <li class="list-group-item"><i class="fa fa-link"></i> <a href="<?php echo element('link_link', $value) ?>" target="_blank"><?php echo html_escape(element('link_link', $value)); ?></a>
                            <?php if (element('show_url_qrcode', element('board', $view))) { ?>
                    ...
                    foreach(explode(',',element('campaign_multi', $view)) as $mvalue) {
                        if(empty($mvalue)) continue;
                    ?>
                        <li class="list-group-item"><i class="fa fa-link"></i> <a href="<?php echo element('link_link', $value)."/".$mvalue ?>" target="_blank"><?php echo html_escape(element('link_link', $value)."/".$mvalue); ?></a><span class="badge ml20" style="float:none;"><a href="<?php echo base_url('stat/real_click_list/'.element('brd_key', element('board',$view)));?>?post_id_[]=<?php echo element('post_id', element('post', $view)); ?>&multi_code=<?php echo $mvalue?>" target="_blank" style="color:inherit;">통계 보러 가기</a> </span>
                            <?php if (element('show_url_qrcode', element('board', $view))) { ?>
                                <span class="url-qrcode"  data-qrcode-url="<?php echo urlencode(element('link_link', $value)); ?>"><i class="fa fa-qrcode"></i></span>
                            <?php } ?>
                        </li>
    ...
    <ul class="list-group">
        ...
        <li class="list-group-item">
            <div class="col-sm-2"> <strong>중간 관리자</strong></div>
            <div class="list-group-item-text" style="min-height:20px;">
            <?php echo element('post_manager', element('post', $view)); ?>
            </div>
        </li>
