    ...
    <?php if (element('link_count', $view) > 0 OR element('file_download_count', $view) > 0) { ?>
        <ul class="list-group">
            ...
            if (element('link_count', $view) > 0) { ?>
                <li class="list-group-item list-group-item-info bold">캠페인 랜딩 URL</li>
            <?php
            foreach (element('link', $view) as $key => $value) {
            ?>
                <li class="list-group-item"><i class="fa fa-link"></i> <a href="<?php echo element('pln_url', $value); ?>" target="_blank"><?php echo html_escape(element('pln_url', $value)); ?></a><span class="badge"><?php echo number_format(element('pln_hit', $value)); ?></span>
                    <?php if (element('show_url_qrcode', element('board', $view))) { ?>
    ...
    <?php if (element('extra_content', $view)) { ?>
            ...
            <?php } ?>
                <li class="list-group-item" style="height:100px;">
                    <div class="col-sm-2" style="height:100%;"> <strong>JS 코드</strong></div>
