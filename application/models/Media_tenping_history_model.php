...
class Media_tenping_history_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'media_tenping_history';
    ...
    {   
        if(!empty($where_in)) $where_in_['media_tenping_history.'.key($where_in)]=$where_in[key($where_in)];
        else $where_in_='';
        
        $select = 'media_tenping_history.*, post.post_title, post.brd_id';
        $join[] = array('table' => 'post', 'on' => 'media_tenping_history.post_id = post.post_id', 'type' => 'left');
