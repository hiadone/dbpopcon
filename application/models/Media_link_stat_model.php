...
class Media_link_stat_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'media_link_stat';
    ...
    {
        $select = 'media_link_stat.*,post.post_id,post.post_title';
        $join[] = array('table' => 'post', 'on' => 'media_link_stat.post_id = post.post_id', 'type' => 'inner');
    ...
    public function get_post_group_list($start_date = '', $end_date = '', $brd_id = 0)
    {
        ...
        $this->db->select('media_link_stat.*,post.post_id,post.post_title', false);
        ...
        $this->db->join('post', 'media_link_stat.post_id = post.post_id', 'inner');

        $this->db->group_by('media_link_stat.post_id');
    ...
    public function get_link_click_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0, $orderby = 'asc', $skey = '', $multi_code = '')
    {
        ...
        if($type==='domain'){
            $this->db->select('SUM(mcs_hit) as hit_cnt, mcs_referrer as day ', false);
        } else if($type==='week'){
            $this->db->select('SUM(mcs_hit) as hit_cnt, WEEKDAY(mcs_datetime) as day ', false);
        } elseif($type==='h'){
            $this->db->select('SUM(mcs_hit) as hit_cnt, mid(mcs_datetime,12,2) as day ', false);
        } else {
            $this->db->select('SUM(mcs_hit) as hit_cnt, left(mcs_datetime, ' . $left . ') as day ', false);
        }
        ...
        if (!empty($skey)) {
            ...
            if(is_array($skey))
                $this->db->where_in('media_link_stat.post_id', $skey);
            else 
                $this->db->where('media_link_stat.post_id', $skey);
        }

        if (!empty($multi_code)) {

            
            $this->db->where('media_link_stat.mcs_code', $multi_code);
        }
    ...
    {   
        ...
        $qry = $this->db->query('SELECT A.post_id,A.brd_id,A.mcs_datetime,A.mcs_hit,A.mcs_code from (SELECT post_id,brd_id,count(*) as mcs_hit, left(mlc_datetime, 13) as mcs_datetime,mlc_code as mcs_code from cb_media_link_log group by post_id,left(mlc_datetime, 13),mlc_code) A ');
