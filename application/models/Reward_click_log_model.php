...
class Reward_click_log_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'reward_click_log';

    /**
     * 사용되는 테이블의 프라이머리키
     */
    public $primary_key = 'rfd_id'; // 사용되는 테이블의 프라이머리키
    ...
    {
        $select = 'reward_click_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname, post.brd_id, post.post_datetime,
            post.post_hit, post.post_secret, post.post_title';
        $join[] = array('table' => 'post', 'on' => 'reward_click_log.post_id = post.post_id', 'type' => 'inner');
    ...
    {   
        if(!empty($where_in)) $where_in_['reward_click_log.'.key($where_in)]=$where_in[key($where_in)];
        else $where_in_='';
        $select = 'reward_click_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname, post.brd_id, post.post_datetime,
            post.post_hit, post.post_secret, post.post_title';
        $join[] = array('table' => 'post', 'on' => 'reward_click_log.post_id = post.post_id', 'type' => 'inner');
