...
class Reward_link_log_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'reward_link_log';

    /**
     * 사용되는 테이블의 프라이머리키
     */
    public $primary_key = 'rlc_id'; // 사용되는 테이블의 프라이머리키
    ...
    {
        $select = 'reward_link_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname,
            post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title, post_link.*';
        $join[] = array('table' => 'post_link', 'on' => 'reward_link_log.pln_id = post_link.pln_id', 'type' => 'inner');
    ...
    {
        if(!empty($where_in)) $where_in_['reward_link_log.'.key($where_in)]=$where_in[key($where_in)];
        else $where_in_='';
        $select = 'reward_link_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname,
            post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title, post_link.*';
        $join[] = array('table' => 'post_link', 'on' => 'reward_link_log.pln_id = post_link.pln_id', 'type' => 'inner');
    ...
    public function get_link_click_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0, $orderby = 'asc')
    {
        ...
        $this->db->select('count(*) as cnt, left(mlc_datetime, ' . $left . ') as day ', false);
        $this->db->where('left(mlc_datetime, 10) >=', $start_date);
        $this->db->where('left(mlc_datetime, 10) <=', $end_date);
        $brd_id = (int) $brd_id;
        if ($brd_id) {
            $this->db->where('brd_id', $brd_id);
        }
        $this->db->group_by('day');
        $this->db->order_by('mlc_datetime', $orderby);
