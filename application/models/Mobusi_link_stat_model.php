...
class Mobusi_link_stat_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'mobusi_link_stat';
    ...
    {
        $select = 'mobusi_link_stat.*,post.post_id,post.post_title';
        $join[] = array('table' => 'post', 'on' => 'mobusi_link_stat.post_id = post.post_id', 'type' => 'inner');
    ...
    public function get_post_group_list($start_date = '', $end_date = '', $brd_id = 0)
    {
        ...
        $this->db->select('mobusi_link_stat.*,post.post_id,post.post_title', false);
        ...
        $this->db->join('post', 'mobusi_link_stat.post_id = post.post_id', 'inner');

        $this->db->group_by('mobusi_link_stat.post_id');
    ...
    public function get_link_click_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0, $orderby = 'asc', $skey = '')
    {
        ...
        if($type==='domain'){
            $this->db->select('SUM(mcs_hit) as hit_cnt, prefijo as day ', false);
        } else if($type==='week'){
        ...
        if (!empty($skey)) {
            ...
            if(is_array($skey))
                $this->db->where_in('mobusi_link_stat.post_id', $skey);
            else 
                $this->db->where('mobusi_link_stat.post_id', $skey);
        }
    ...
    {   
        ...
        $qry = $this->db->query('SELECT A.post_id,A.brd_id,A.mcs_datetime,A.prefijo,A.mcs_hit from (SELECT post_id,brd_id,count(*) as mcs_hit, left(mlc_datetime, 13) as mcs_datetime,prefijo as prefijo from cb_mobusi_link_log group by post_id,left(mlc_datetime, 13),prefijo) A ');
