...
class Shortcut_stat_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'shortcut_stat';

    /**
     * 사용되는 테이블의 프라이머리키
     */
    public $primary_key = 'sc_id'; // 사용되는 테이블의 프라이머리키
    ...
    {
        $select = 'shortcut_stat.*,post.post_id,post.post_title';
        $join[] = array('table' => 'post', 'on' => 'shortcut_stat.post_id = post.post_id', 'type' => 'inner');
    ...
    public function get_post_group_list($start_date = '', $end_date = '', $brd_id = 0)
    {
        ...
        $this->db->select('shortcut_stat.*,post.post_id,post.post_title', false);
        ...
        $this->db->join('post', 'shortcut_stat.post_id = post.post_id', 'inner');

        $this->db->group_by('shortcut_stat.post_id');
    ...
    public function get_link_click_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0, $orderby = 'asc', $skey = '')
    {
        ...
        if($type==='domain'){
            $this->db->select('SUM(sc_cnt) as cnt,sc_hit as hit_cnt, sc_referrer as day ', false);
        } elseif($type==='week'){
            $this->db->select('SUM(sc_cnt) as cnt,sc_hit as hit_cnt, WEEKDAY(sc_datetime) as day ', false);
        } elseif($type==='h'){
            $this->db->select('SUM(sc_cnt) as cnt,sc_hit as hit_cnt, mid(sc_datetime,12,2) as day ', false);
        } else {
            $this->db->select('SUM(sc_cnt) as cnt,sc_hit as hit_cnt, left(sc_datetime, ' . $left . ') as day ', false);
        }
        ...
        if (!empty($skey)) {
            ...
            if(is_array($skey))
                $this->db->where_in('shortcut_stat.post_id', $skey);
            else 
                $this->db->where('shortcut_stat.post_id', $skey);
        }
