...
class Media_link_log_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'media_link_log';
    ...
    {
        $select = 'media_link_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname,
            post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title, post_link.*';
        $join[] = array('table' => 'post_link', 'on' => 'media_link_log.pln_id = post_link.pln_id', 'type' => 'inner');
    ...
    {
        if(!empty($where_in)) $where_in_['media_link_log.'.key($where_in)]=$where_in[key($where_in)];
        else $where_in_='';
        $select = 'media_link_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname,
            post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title, post_link.*';
        $join[] = array('table' => 'post_link', 'on' => 'media_link_log.pln_id = post_link.pln_id', 'type' => 'inner');
