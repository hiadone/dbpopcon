...
class Media_click_stat_model extends CB_Model
{
    ...
    public function get_link_click_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0, $orderby = 'asc', $skey = '', $multi_code = '')
    {
        ...
        if($type==='domain'){
            $this->db->select('SUM(mds_cnt) as cnt, mds_referrer as day ', false);
        } elseif($type==='week'){
        ...
        if (!empty($multi_code)) {
            ...
            $this->db->where('media_click_stat.mds_code', $multi_code);
        }
    ...
    {   
        ...
        $qry = $this->db->query('SELECT A.post_id,A.brd_id,A.mds_datetime,A.mds_cnt,A.mds_code from (SELECT post_id,brd_id,count(*) as mds_cnt, left(mfd_datetime, 13) as mds_datetime, mfd_code as mds_code from cb_media_click_log group by post_id,left(mfd_datetime, 13),mfd_code) A ');
