...
class Media_multiple_history_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'media_multiple_history';
    ...
    {   
        if(!empty($where_in)) $where_in_['media_multiple_history.'.key($where_in)]=$where_in[key($where_in)];
        else $where_in_='';
        
        $select = 'media_multiple_history.*, post.post_title, post.brd_id';
        $join[] = array('table' => 'post', 'on' => 'media_multiple_history.post_id = post.post_id', 'type' => 'left');
