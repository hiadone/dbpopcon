...
class Post_shortcut_click_log_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'post_shortcut_click_log';

    /**
     * 사용되는 테이블의 프라이머리키
     */
    public $primary_key = 'slc_id'; // 사용되는 테이블의 프라이머리키
    ...
    {
        $select = 'post_shortcut_click_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname,
            post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title, post_link.*';
        $join[] = array('table' => 'post_link', 'on' => 'post_shortcut_click_log.pln_id = post_link.pln_id', 'type' => 'inner');
    ...
    public function get_shortcut_click_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0, $orderby = 'asc')
    {
