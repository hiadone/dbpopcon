...
class Shortcut_file_download_log_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'shortcut_file_download_log';

    /**
     * 사용되는 테이블의 프라이머리키
     */
    public $primary_key = 'sfd_id'; // 사용되는 테이블의 프라이머리키
    ...
    {
        $select = 'shortcut_file_download_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname, post.brd_id, post.post_datetime,
        ...
        $join[] = array('table' => 'post_file', 'on' => 'shortcut_file_download_log.sfi_id = post_file.pfi_id', 'type' => 'inner');
    ...
    {   
        if(!empty($where_in)) $where_in_['shortcut_file_download_log.'.key($where_in)]=$where_in[key($where_in)];
        else $where_in_='';
        $select = 'shortcut_file_download_log.*, post.mem_id as post_mem_id, post.post_userid, post.post_nickname, post.brd_id, post.post_datetime,
            post.post_hit, post.post_secret, post.post_title, post_file.pfi_id, post_file.pfi_originname, post_file.pfi_filename, post_file.pfi_download,
            post_file.pfi_filesize, post_file.pfi_width, post_file.pfi_height, post_file.pfi_type, post_file.pfi_is_image, post_file.pfi_datetime, post_file.pfi_ip';
    ...
    public function get_post_click_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0, $orderby = 'asc')
    {
        ...
        $this->db->select('count(*) as cnt, left(sfd_datetime, ' . $left . ') as day ', false);
        $this->db->where('left(sfd_datetime, 10) >=', $start_date);
        $this->db->where('left(sfd_datetime, 10) <=', $end_date);
        $brd_id = (int) $brd_id;
        if ($brd_id) {
            $this->db->where('brd_id', $brd_id);
        }
        $this->db->group_by('day');
        $this->db->order_by('sfd_datetime', $orderby);
