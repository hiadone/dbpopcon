...
class Board extends CI_Controller
{
    ...
    public function latest($config)
    {
        ...
        if (element('use_personal', $view['view']['board']) && $is_admin === false) {
    ...
    public function media_latest($config)
    {   
        
        $post_md = element('post_md', $config);
        $brd_key = element('brd_key', $config);
        ...
        $skin = element('brd_key', $config).'/'.element('post_md', $config);

        if ($cache_minute> 0) {
            $cache_post_md = is_array($post_md) ? implode('-', $post_md) : $post_md;
            $cache_brd_key = is_array($brd_key) ? implode('-', $brd_key) : $brd_key;
            
            $cachename = 'media/latest-s-' . $brd_key."-".$post_md . '-i-' . $cache_post_md . '-k-' . $cache_brd_key . '-l-k-l-t-g-w-' . $image_width . '-h-' . $image_height . '-p';
            $html = $this->CI->cache->get($cachename);
            if ($html) {
                return $html;
            }
        ...
        $this->CI->load->model( array('Post_model',  'Post_extra_vars_model'));
        ...
        if (element('post_file', $post) OR element('post_image', $post)) {
            $this->CI->load->model('Post_file_model');
            $filewhere = array(
                'post_id' => element('post_id', $post),
            );
            $view['view']['file'] = $file = $this->CI->Post_file_model
        ...
        //$this->db->cache_off();
        $userAgent = $this->CI->agent->agent_string() ? $this->CI->agent->agent_string() : '';        
        ...
        $html = $this->CI->load->view('media/' . $skin . '/index_latest', $view, true);
