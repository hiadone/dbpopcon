...
class Stat extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Shortcut_file_download_stat','Shortcut_link_click_stat','Shortcut_file_download_log','Shortcut_link_click_log','Media_link_stat','Media_link_log','Media_click_stat','Media_click_log','Mobusi_click_stat','Mobusi_link_stat','Mobusi_click_log','Mobusi_link_log', 'Board', 'Post','Post_link','Post_meta','Post_extra_vars','Member_group_member','Member_group');   
    ...
    //     $this->{ucfirst($this->link_log_model)}->allow_searcfield = array('sfd_id', 'post.post_title', 'post.post_id'); // 검색이 가능한 필드
    //     $this->{ucfirst($this->link_log_model)}->search_field_equal = array('sfd_id', 'post.post_title', 'post.post_id'); // 검색중 like 가 

    //     $skey = $this->input->get('post_id_', null, '');
        

    //     $result = $this->{ucfirst($this->link_log_model)}->get_link_click_count($datetype, $start_date, $end_date, $brd_id, $orderby, $skey);

    //     $result_click = $this->{ucfirst($this->link_log_model)}->get_link_click_count($datetype, $start_date, $end_date, $brd_id, $orderby, $skey);
    ...
    //     $post_result = $this->Media_stat_list_model
    ...
    public function migration(){
       ...
       // $result = $this->Media_stat_list_model->migration();
        
        // foreach($result as $value){
        //     $this->Media_stat_model->replace($value);
        // }
        ...
        echo $criterion;
        $deletewhere = array(
                    'sc_datetime <=' => $criterion,
                );

       // $result = $this->Media_stat_list_model->delete_where($deletewhere);
    ...
    public function get_member_group_post_list($brd_key='')
    {
        ...
        $mgr_id=array('shortcut'=>1,'mobusi'=>1,'selfcert_ad'=>2,'linkmine'=>5,'tenping' => 6,'linkasia' => 7 ,'nomal_campaign' => 8,'viashare' => 10,'multiple' => 1,'b-a-1' => 1);
        $where = array();
        $where['post_del <>'] = 2;
        $where['post_notice'] = 0;
    ...
    {
        if($brd_key==='shortcut'){
            $this->link_log_model='shortcut_link_click_log_model';
            $this->link_stat_model='shortcut_link_click_stat_model';
            $this->click_log_model='shortcut_file_download_log_model';
            $this->click_stat_model='shortcut_file_download_stat_model';
            $this->id_key='s';
        }

        if($brd_key==='selfcert_ad' || $brd_key==='linkmine' || $brd_key==='tenping' || $brd_key==='linkasia' || $brd_key==='nomal_campaign' || $brd_key==='multiple' || $brd_key==='viashare' || $brd_key==='b-a-1'){
            $this->link_log_model='media_link_log_model';
            $this->link_stat_model='media_link_stat_model';
            $this->click_log_model='media_click_log_model';
            $this->click_stat_model='media_click_stat_model';
            $this->id_key='m';
        }
        if($brd_key==='mobusi'){
            $this->link_log_model='mobusi_link_log_model';
            $this->link_stat_model='mobusi_link_stat_model';
            $this->click_log_model='mobusi_click_log_model';
            $this->click_stat_model='mobusi_click_stat_model';
            $this->id_key='m';
        }
    }
}
