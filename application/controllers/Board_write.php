...
class Board_write extends CB_Controller
{
    ...
    public function _write_common($board, $origin = '', $reply = '')
    {
        ...
        if ($form_validation === false OR $file_error) {
            ...
            if ($form && is_array($form)) {
                foreach ($form as $key => $value) {
                    ...
                    if (element('field_type', $value) === 'text'
                        ...
                        if(element('field_name', $value)==='campaign_manager'){
                                $extra_content[$k]['input'].='<span> 중간관리자 아이디를 ,(콤마) 로 구분하여 입력해 주세요</span>';
                        }
                            ...
                            if(element('field_name', $value)==='campaign_multi'){
                                $extra_content[$k]['input'].='<span> 코드는 ,(콤마) 로 구분하여 입력해 주세요</span>';
                            }
            ...
            $post_manager = $this->input->post('post_manager', null, '');
            $post_js = '';
            ...
            if ($form && is_array($form)) {
                foreach ($form as $key => $value) {
                    ...
                    if(element('field_name', $value) == "campaign_multi")
                        $extradata[element('field_name', $value)] = str_replace(" ","",$this->input->post(element('field_name', $value), null, ''));
                    else
