...
class Board_post extends CB_Controller
{
    ...
    public function post($post_id = 0, $print = false)
    {
        ...
        if (element('use_personal', $board) && $is_admin === false
            && $mem_id !== abs(element('mem_id', $post)) && strpos(element('post_manager', $post),$this->member->item('mem_userid')) === false) {
        ...
        if (element('post_link_count', $post)) {
            ...
            if ($link && is_array($link)) {
                foreach ($link as $key => $value) {
                    if(element('brd_key', $board) === 'mobusi' )
                        $view['view']['link'][$key]['link_link'] = site_url('postact/mobusi_link/' . element('pln_id', $value));
                    elseif(element('bgr_id', $board)==='2')
                        $view['view']['link'][$key]['link_link'] = site_url('postact/media_link/' . element('pln_id', $value));
                    elseif(element('bgr_id', $board)==='8')
                        $view['view']['link'][$key]['link_link'] = site_url('postact/media_multi_link/' . element('pln_id', $value));
                    elseif(element('bgr_id', $board)==='9')
                        $view['view']['link'][$key]['link_link'] = site_url('postact/reward_link/' . element('pln_id', $value));
                    else  
        ...
        if (element('post_del', $post)) {
            ...
            if ($form && is_array($form)) {
                foreach ($form as $key => $value) {
                    ...
                    if(element('field_name', $value) ==='adv_key')
                        $view['view']['adv_key'] = element(element('field_name', $value), element('extravars', $post));
                    ...
                    if(element('field_name', $value) ==='campaign_multi') $view['view']['campaign_multi'] = $item;
                }
            }
        ...
        if($post_id === 525) $show_list_from_view = false ;
        ...
        if ($use_prev_next) {
            ...
            if (element('use_personal', $board) && $is_admin === false) {
                ...
                $this->Post_model->personalwhere($mem_id,$this->member->item('post_manager'));
            }
        ...
        if ($show_list_from_view) {
            if(element('brd_key', $board)==='selfcert_ad' )
                $view['view']['list'] = $list = $this->get_selfcert_history(element('brd_key', $board), element('post_id', $post));            
            elseif(element('brd_key', $board) === 'linkmine'){
                $view['view']['list'] = $list = $this->get_linkmine_history(element('brd_key', $board), element('post_id', $post),$this->input->get('linkminestatus',null,0));
                $view['view']['api_list']  = $this->get_linkmine_history_api_flag(element('brd_key', $board), element('post_id', $post));

            }
            elseif(element('brd_key', $board) === 'tenping' || element('brd_key', $board) === 'nomal_campaign'){
                $view['view']['list'] = $list = $this->get_tenping_history(element('brd_key', $board), element('post_id', $post),$this->input->get('tenpingstatus',null,0));
                $view['view']['api_list']  = $this->get_tenping_history_api_flag(element('brd_key', $board), element('post_id', $post));


            }
            elseif(element('brd_key', $board) === 'linkasia'){
                $view['view']['list'] = $list = $this->get_linkasia_history(element('brd_key', $board), element('post_id', $post),$this->input->get('linkasiastatus',null,0));
                $view['view']['api_list']  = $this->get_linkasia_history_api_flag(element('brd_key', $board), element('post_id', $post));


            }elseif(element('brd_key', $board) === 'viashare'){
                $view['view']['list'] = $list = $this->get_viashare_history(element('brd_key', $board), element('post_id', $post),$this->input->get('viasharestatus',null,0));
                $view['view']['api_list']  = $this->get_viashare_history_api_flag(element('brd_key', $board), element('post_id', $post));


            } elseif(element('brd_key', $board) === 'multiple'){
                $view['view']['list'] = $list = $this->get_multiple_history(element('brd_key', $board), element('post_id', $post),$this->input->get('multiplestatus',null,0),$this->input->get('multi_code',null,''));
                $view['view']['api_list']  = $this->get_multiple_history_api_flag(element('brd_key', $board), element('post_id', $post));


            } elseif(element('brd_key', $board) === 'b-a-1'){
                $view['view']['list'] = $list = $this->get_tenping_multiple_history(element('brd_key', $board), element('post_id', $post),$this->input->get('multiplestatus',null,0),$this->input->get('multi_code',null,''));
                $view['view']['api_list']  = $this->get_tenping_multiple_history_api_flag(element('brd_key', $board), element('post_id', $post));


            } elseif(element('brd_key', $board) === 'b-a-2'){
                $view['view']['list'] = $list = $this->get_multiple_history(element('brd_key', $board), element('post_id', $post),$this->input->get('multiplestatus',null,0),$this->input->get('multi_code',null,''));
                $view['view']['api_list']  = $this->get_multiple_history_api_flag(element('brd_key', $board), element('post_id', $post));
        ...
        if ($print === false) {
            ...
            // 이벤트가 존재하면 실행합니다
            $view['view']['event']['before_post_layout'] = Events::trigger('after_'.element('brd_key', $board).'_'.element('post_md', $post).'_js', $eventname);
            ...
            if ($show_list_from_view) {
                ...
                if(element('bgr_id', $board)==='2' || element('bgr_id', $board)==='8' || element('bgr_id', $board)==='9')
    ...
    public function _get_list($brd_key, $from_view = '')
    {
        ...
        if (element('use_personal', $board) && $is_admin === false) {
            ...
            $this->Post_model->personalwhere($mem_id,$this->member->item('mem_userid'));
        }
        ...
        if ($skeyword) {
            ...
            $return['list_url'] = board_url(element('brd_key', $board) . '?' . $param->replace('page_sub'));
    ...
    public function get_selfcert_history($brd_key,$post_id)
    {
        ...
        $findex = $this->input->get('findex') ? $this->input->get('findex') : $this->Media_selfcert_history_model->primary_key;
        ...
        $where['media_selfcert_history.post_id'] = $post_id;
        // $where['msh_status'] = 1;
        
        

        $this->db->select('count(msh_id) as cnt,msh_event_result');
        $this->db->from('media_selfcert_history');
        ...
        foreach($result as $value){
            $category[element('msh_event_result',$value)] = element('cnt',$value);
            if(empty($category['total']))
                $category['total'] = element('cnt',$value);
            else 
                $category['total'] += element('cnt',$value);
        }
        

        if($this->input->get('msh_event_result')==='all') $where['msh_event_result <>'] = 0;        
        elseif($this->input->get('msh_event_result')) $where['msh_event_result'] = $this->input->get('msh_event_result');

        if($this->input->get('msh_val1')==='none') $where['msh_val1'] = '';        
        elseif($this->input->get('msh_val1')) $where['msh_val1'] = $this->input->get('msh_val1');
        ...
        $this->Media_selfcert_history_model->allow_search_field = array('msh_name','msh_mobileno','msh_mem_id','msh_dupinfo','msh_adid'); // 검색이 가능한 필드
        //$this->{$this->modelname}->search_field_equal = array('msh_name','msh_mobileno'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->Media_selfcert_history_model->allow_order_field = array('msh_name'); // 정렬이 가능한 필드
        $result = $this->Media_selfcert_history_model
        ...
        if (element('list', $result)) {
            ...
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['display_datetime'] = element('msh_event_datetime', $val) ? display_datetime(element('msh_event_datetime', $val)) : display_datetime(element('msh_datetime', $val));
         ...
         * primary key 정보를 저장합니다
         */
        $return['primary_key'] = $this->Media_selfcert_history_model->primary_key;
    ...
    public function excel_selfcert_history($brd_key,$post_id)
    {
       ...
       if (element('list', $result)) {
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = $this->Media_selfcert_history_model->primary_key;
        ...
        header('Content-Disposition: attachment; filename=이벤트참여정보_' . cdate('Y_m_d') . '.xls');
        echo $this->load->view('/board/'.$skin_dir.'/excel_'.$brd_key, $view, true);
    }

    public function get_linkmine_history($brd_key,$post_id,$linkminestatus=0)
    {
        ...
        $param =& $this->querystring;
        $page = (((int) $this->input->get('page_sub')) > 0) ? ((int) $this->input->get('page_sub')) : 1;
        $findex = $this->input->get('findex') ? $this->input->get('findex') : $this->Media_linkmine_history_model->primary_key;
        $forder = $this->input->get('forder', null, 'desc');
        $sfield = $this->input->get('sfield', null, '');
        $skeyword = $this->input->get('skeyword', null, '');
        ...
        $where['media_linkmine_history.post_id'] = $post_id;
        if(!empty($linkminestatus)) $where['mlh_status'] = $linkminestatus;
        ...
        $this->Media_linkmine_history_model->allow_search_field = array('mlh_name','mlh_mobileno'); // 검색이 가능한 필드
        //$this->{$this->modelname}->search_field_equal = array('msh_name','msh_mobileno'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->Media_linkmine_history_model->allow_order_field = array('mlh_name'); // 정렬이 가능한 필드
        $result = $this->Media_linkmine_history_model
            ->get_admin_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword,'');
        $list_num = $result['total_rows'] - ($page - 1) * $per_page;
        ...
        if (element('list', $result)) {
            ...
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['display_name'] = element('mlh_name', $val);
                $result['list'][$key]['display_datetime'] = display_datetime(element('mlh_datetime', $val));
                $result['list'][$key]['display_rst2_datetime'] = display_datetime(element('mlh_rst2_date', $val));
                $result['list'][$key]['num'] = $list_num--;
            }
         ...
         * primary key 정보를 저장합니다
         */
        $return['primary_key'] = $this->Media_linkmine_history_model->primary_key;
    ...
    public function excel_linkmine_history($brd_key,$post_id)
    {
       ...
       if (element('list', $result)) {
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = $this->Media_linkmine_history_model->primary_key;
    ...
    public function get_linkmine_history_api_flag($brd_key,$post_id)
    {
    ...
    public function get_tenping_history($brd_key,$post_id,$tenpingstatus=0)
    {
        ...
        $this->load->model('Tenping_extra_vars_model');

        $param =& $this->querystring;
        $page = (((int) $this->input->get('page_sub')) > 0) ? ((int) $this->input->get('page_sub')) : 1;
        $findex = $this->input->get('findex') ? $this->input->get('findex') : $this->Media_tenping_history_model->primary_key;
        $forder = $this->input->get('forder', null, 'desc');
        $sfield = $this->input->get('sfield', null, '');
        $skeyword = $this->input->get('skeyword', null, '');
        ...
        $where['media_tenping_history.post_id'] = $post_id;
        if(!empty($tenpingstatus)) $where['mlh_status'] = $tenpingstatus;
        ...
        $this->Media_tenping_history_model->allow_search_field = array('mlh_name','mlh_mobileno'); // 검색이 가능한 필드
        //$this->{$this->modelname}->search_field_equal = array('msh_name','msh_mobileno'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->Media_tenping_history_model->allow_order_field = array('mlh_name'); // 정렬이 가능한 필드
        $result = $this->Media_tenping_history_model
            ->get_admin_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword,'');
        $list_num = $result['total_rows'] - ($page - 1) * $per_page;
        ...
        if (element('list', $result)) {
            ...
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['display_name'] = element('mlh_name', $val);
                $result['list'][$key]['display_datetime'] = display_datetime(element('mlh_datetime', $val),'full');
                
                $result['list'][$key]['num'] = $list_num--;
                $result['list'][$key]['display_rst2_datetime'] = display_datetime(element('mlh_rst2_date', $val));


                $result['list'][$key]['tenping_extra_vars']= $this->Tenping_extra_vars_model->get_all_meta(element('mlh_id', $val));
                if($result['list'][$key]['tenping_extra_vars']) $result['tenping_extra_vars'] = true;
            }
         ...
         * primary key 정보를 저장합니다
         */
        $return['primary_key'] = $this->Media_tenping_history_model->primary_key;
    ...
    public function get_tenping_multiple_history($brd_key,$post_id,$multiplestatus=0,$multi_code='')
    {
        ...
        $param =& $this->querystring;
        $page = (((int) $this->input->get('page_sub')) > 0) ? ((int) $this->input->get('page_sub')) : 1;
        $findex = $this->input->get('findex') ? $this->input->get('findex') : $this->Media_multiple_history_model->primary_key;
        $forder = $this->input->get('forder', null, 'desc');
        $sfield = $this->input->get('sfield', null, '');
        $skeyword = $this->input->get('skeyword', null, '');
        ...
        $where['media_multiple_history.post_id'] = $post_id;
        if(!empty($multiplestatus)) $where['mlh_status'] = $multiplestatus;
        ...
        $this->Media_multiple_history_model->allow_search_field = array('mlh_name','mlh_mobileno'); // 검색이 가능한 필드
        //$this->{$this->modelname}->search_field_equal = array('msh_name','msh_mobileno'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->Media_multiple_history_model->allow_order_field = array('mlh_name'); // 정렬이 가능한 필드
        $result = $this->Media_multiple_history_model
            ->get_admin_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword,'');
        $list_num = $result['total_rows'] - ($page - 1) * $per_page;
         ...
         * primary key 정보를 저장합니다
         */
        $return['primary_key'] = $this->Media_multiple_history_model->primary_key;
    ...
    public function excel_tenping_multiple_history($brd_key,$post_id)
    {
       ...
       if (element('list', $result)) {
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = $this->Media_multiple_history_model->primary_key;
    ...
    public function get_tenping_multiple_history_api_flag($brd_key,$post_id)
    {
    ...
    public function excel_tenping_history($brd_key,$post_id)
    {
       ...
       if (element('list', $result)) {
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = $this->Media_tenping_history_model->primary_key;
    ...
    public function get_tenping_history_api_flag($brd_key,$post_id)
    {
    ...
    public function get_linkasia_history($brd_key,$post_id,$linkasiastatus=0)
    {
        ...
        $param =& $this->querystring;
        $page = (((int) $this->input->get('page_sub')) > 0) ? ((int) $this->input->get('page_sub')) : 1;
        $findex = $this->input->get('findex') ? $this->input->get('findex') : $this->Media_linkasia_history_model->primary_key;
        $forder = $this->input->get('forder', null, 'desc');
        $sfield = $this->input->get('sfield', null, '');
        $skeyword = $this->input->get('skeyword', null, '');
        ...
        $where['media_linkasia_history.post_id'] = $post_id;
        if(!empty($linkasiastatus)) $where['mlh_status'] = $linkasiastatus;
        ...
        $this->Media_linkasia_history_model->allow_search_field = array('mlh_name','mlh_mobileno'); // 검색이 가능한 필드
        //$this->{$this->modelname}->search_field_equal = array('msh_name','msh_mobileno'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->Media_linkasia_history_model->allow_order_field = array('mlh_name'); // 정렬이 가능한 필드
        $result = $this->Media_linkasia_history_model
            ->get_admin_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword,'');
        $list_num = $result['total_rows'] - ($page - 1) * $per_page;
         ...
         * primary key 정보를 저장합니다
         */
        $return['primary_key'] = $this->Media_linkasia_history_model->primary_key;
    ...
    public function excel_linkasia_history($brd_key,$post_id)
    {
       ...
       if (element('list', $result)) {
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = $this->Media_linkasia_history_model->primary_key;
    ...
    public function get_linkasia_history_api_flag($brd_key,$post_id)
    {
    ...
    public function get_multiple_history($brd_key,$post_id,$multiplestatus=0,$multi_code='')
    {
        ...
        $this->db->select('count(mlh_id) as cnt,multi_code');
        $this->db->from('media_multiple_history');
        $this->db->where(array('post_id'=> $post_id));
        $this->db->group_by('multi_code');
        $qry = $this->db->get();
        $result = $qry->result_array();
        $category  = array();
        foreach($result as $value){
            $category[element('multi_code',$value)] = element('cnt',$value);
            if(empty($category['total']))
                $category['total'] = element('cnt',$value);
            else 
                $category['total'] += element('cnt',$value);
        ...
        if(!empty($multi_code)) $where['multi_code'] = $multi_code;
    ...
    public function excel_multiple_history($brd_key,$post_id)
    {
    ...
    public function get_multiple_history_api_flag($brd_key,$post_id)
    {
    ...
    public function get_viashare_history($brd_key,$post_id,$viasharestatus=0)
    {
        ...
        $param =& $this->querystring;
        $page = (((int) $this->input->get('page_sub')) > 0) ? ((int) $this->input->get('page_sub')) : 1;
        $findex = $this->input->get('findex') ? $this->input->get('findex') : $this->Media_viashare_history_model->primary_key;
        $forder = $this->input->get('forder', null, 'desc');
        $sfield = $this->input->get('sfield', null, '');
        $skeyword = $this->input->get('skeyword', null, '');
        ...
        $where['media_viashare_history.post_id'] = $post_id;
        if(!empty($viasharestatus)) $where['mlh_status'] = $viasharestatus;
        ...
        $this->Media_viashare_history_model->allow_search_field = array('mlh_name','mlh_mobileno'); // 검색이 가능한 필드
        //$this->{$this->modelname}->search_field_equal = array('msh_name','msh_mobileno'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->Media_viashare_history_model->allow_order_field = array('mlh_name'); // 정렬이 가능한 필드
        $result = $this->Media_viashare_history_model
            ->get_admin_list($per_page, $offset, $where, '', $findex, $forder, $sfield, $skeyword,'');
        $list_num = $result['total_rows'] - ($page - 1) * $per_page;
         ...
         * primary key 정보를 저장합니다
         */
        $return['primary_key'] = $this->Media_viashare_history_model->primary_key;
    ...
    public function excel_viashare_history($brd_key,$post_id)
    {
       ...
       if (element('list', $result)) {
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = $this->Media_viashare_history_model->primary_key;
    ...
    public function get_viashare_history_api_flag($brd_key,$post_id)
    {
