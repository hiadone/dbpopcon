 ...
 */
class Media_board_write extends CB_Controller
{
    ...
    public function _write_common($board, $origin = '', $reply = '')
    {
                ...
                foreach ($_FILES['post_file']['name'] as $i => $value) {
                    if ($value) {
                        ...
                        if ($this->upload->do_upload()) {
                            ...
                            if(element('bgr_id', $board)==='2' || element('bgr_id', $board)==='8'){

                                // If they included the .zip file extension we'll remove it
                                if (preg_match('|.+?\.zip$|', element('file_name', $filedata)) && $this->input->post('post_md')){
                                    // Load the Zip class and output it
                                    $Zip = new ZipArchive;
                                    if($Zip->open(FCPATH.$upload_path.element('file_name', $filedata))){
                                        if(!$Zip->extractTo(FCPATH.'views/media/'.element('brd_key',$board).'/'.$this->input->post('post_md').'/')){
                                            alert('업로드한 파일에 문제가 있습니다. 관리자에게 문의하세요');
                                        }
                                        $Zip->close();
                                    }
        ...
        if ($form_validation === false OR $file_error) {
            ...
            $default['campaign_type']='상관없음';
            $default['disable_hours']=24;
            $default['disable_hours']=24;
            $default['execute_time']=0;
            $default['visit_cnt']=1;
            $default['campaign_gender']='제한없음';
            if ($form && is_array($form)) {
                foreach ($form as $key => $value) {
                    ...
                    if (element('field_type', $value) === 'text'
                        ...
                        OR element('field_type', $value) === 'number') {

                        if(element('field_name', $value)==='campaign_age'){
                            $css_class ='px150';
                        }

                        if (element('field_type', $value) === 'date') {
                            $extra_content[$k]['input'] .= '<input type="text" id="' . element('field_name', $value) . '" name="' . element('field_name', $value) . '" class="form-control input datepicker px150" value="' . set_value(element('field_name', $value)) . '" readonly="readonly" ' . $required . ' />';
                            ...
                            $extra_content[$k]['input'] .= '<input type="' . element('field_type', $value) . '" id="' . element('field_name', $value) . '" name="' . element('field_name', $value) . '" class="form-control input '.$css_class.'" value="' . set_value(element('field_name', $value)) . '" ' . $required . '/>';
                        }

                        if(element('field_name', $value)==='disable_hours'){
                            $extra_content[$k]['input'].='<span> 캠페인 설치 주기를 시간 단위로 입력하세요</span>';
                        }

                        if(element('field_name', $value)==='execute_time'){
                            $extra_content[$k]['input'].='<span> 캠페인 실행타이머를 초 단위로 입력하세요 </span>';
                        }

                        if(element('field_name', $value)==='visit_cnt'){
                            $extra_content[$k]['input'].='<span> 주기당 캠페인 설치횟수를 입력하세요</span>';
                        }

                        if(element('field_name', $value)==='campaign_startdate'){
                            $extra_content[$k]['input'].='<span> 입력 하지 않으면 즉시</span>';
                        }

                        if(element('field_name', $value)==='campaign_enddate'){
                            $extra_content[$k]['input'].='<span> 입력 하지 않으면 무한</span>';
                        }
                        if(element('field_name', $value)==='campaign_age'){
                            $extra_content[$k]['input'].='<span> ~ 구분 하여 입력 <small>ex) 20 ~ 60 또는  ~ 20 or 20 ~ </small></span>';
                        }
                            ...
                            if(element('field_name', $value)==='campaign_multi'){
                                $extra_content[$k]['input'].='<span> 코드는 ,(콤마) 로 구분 해주세요</span>';
                            }
            ...
            $this->view = array(
                ...
                'media/' . $listskindir . '/' . $list_skin_file,
            );
            ...
            $post_js = '';
            if(element('bgr_id', $board) === '1' ){
                
                $post_js = urlencode('(function() {
                  function async_load(){
                    var s = document.createElement("script");
                    s.type = "text/javascript";
                    s.async = true;
                    s.id="hiadone_shortcut";
                    s.src = "http://shortcut.dbpopcon.com/views/shortcut/shortcut.js?brd_key='.element('brd_key', $board).'&post_md='.$post_md.'";
            ...
            if ($mem_id) {
                if (element('use_anonymous', $board)) {
                    ...
                    $updatedata['post_userid'] = $this->member->item('mem_userid',null,'');
            ...
            if ($post_link && is_array($post_link) && count($post_link) > 0) {
                foreach ($post_link as $pkey => $pval) {
                    if ($pval|| element('bgr_id', $board)==='2' || element('bgr_id', $board)==='8' || element('bgr_id', $board)==='9') {

                        if(element('bgr_id', $board)==='2'){
                            $linkupdate = array(
                                'post_id' => $post_id,
                                'brd_id' => element('brd_id', $board),
                                'pln_url' => prep_url(base_url('/media/'.element('brd_key',$board).'/'.$post_md)),
                            );

                            
                        } else if(element('bgr_id', $board)==='8'){


                            $linkupdate = array(
                                'post_id' => $post_id,
                                'brd_id' => element('brd_id', $board),
                                'pln_url' => prep_url(base_url('/media_multiple/'.element('brd_key',$board).'/'.$post_md)),
                            );

                        } else if(element('bgr_id', $board)==='9'){


                            $linkupdate = array(
                                'post_id' => $post_id,
                                'brd_id' => element('brd_id', $board),
                                'pln_url' => prep_url(base_url('/reward/'.element('brd_key',$board).'/'.$post_md)),
                            );
            ...
            $redirecturl = site_url('media_write/'.element('brd_key', $board)). '?' . $param->output();
    ...
    public function modify($post_id = 0)
    {
        ...
        $view['view']['cancel_url'] = site_url('media_write/'.element('brd_key', $board)). '?' . $param->output();
        ...
        if ($file && is_array($file)) {
            foreach ($file as $key => $value) {
                ...
                $view['view']['file'][$key]['thumb_url'] = thumb_url('post', element('pfi_filename', $value));
            }
        }
                        ...
                        if ($this->upload->do_upload()) {
                            ...
                            if(element('bgr_id', $board)==='2' || element('bgr_id', $board)==='8'){
                                if (preg_match('|.+?\.zip$|', element('file_name', $filedata)) && $this->input->post('post_md')){
                                    ...
                                    $Zip = new ZipArchive;
                                    if($Zip->open(FCPATH.$upload_path.element('file_name', $filedata))){
                                        @delete_files(FCPATH.'views/media/'.element('brd_key',$board).'/'.$this->input->post('post_md').'/',ture);
                                        if(!$Zip->extractTo(FCPATH.'views/media/'.element('brd_key',$board).'/'.$this->input->post('post_md').'/')){
                                            alert('업로드한 파일에 문제가 있습니다. 관리자에게 문의하세요');
                                        }
                                        $Zip->close();
        ...
        if ($form_validation === false OR $file_error) {
            ...
            if ($form && is_array($form)) {
                foreach ($form as $key => $value) {
                    ...
                    if (element('field_type', $value) === 'text'
                        ...
                        OR element('field_type', $value) === 'number') {
                        if (element('field_type', $value) === 'date') {
                            $extra_content[$k]['input'] .= '<input type="text" id="' . element('field_name', $value) . '" name="' . element('field_name', $value) . '" class="form-control input datepicker px150" value="' . set_value(element('field_name', $value), $item) . '" readonly="readonly" ' . $required . ' />';
                        } elseif (element('field_type', $value) === 'phone') {
                        ...
                        if(element('field_name', $value)==='disable_hours'){
                            $extra_content[$k]['input'].='<span> 캠페인 실행 주기를 시간 단위로 입력하세요</span>';
                        }
                        ...
                        if(element('field_name', $value)==='visit_cnt'){
                            $extra_content[$k]['input'].='<span> 주기당 캠페인 실행횟수를 입력하세요</span>';
                        }
                        ...
                        if(element('field_name', $value)==='campaign_age'){
                            $extra_content[$k]['input'].='<span> ~ 구분 하여 입력 <small>ex) 20 ~ 60 or ~ 20 or 20 ~ </small></span>';
                        }
            ...
            if ($use_upload && $this->input->post('post_file_del')) {
                foreach ($this->input->post('post_file_del') as $key => $val) {
                    if ($val === '1' && ! isset($uploadfiledata2[$key])) {
                        ...
                        @unlink(config_item('uploads_dir') .  '/post/' . element('pfi_filename', $oldpostfile));
                        if(element('bgr_id', $board)==='2' || element('bgr_id', $board)==='8'){
                            @delete_files(FCPATH.'views/media/'.element('brd_key',$board).'/'.$post_md.'/',ture);
                        }
            ...
            // $this->db->cache_delete('shortcut',element('brd_key', $board));
    ...
    public function _get_list($brd_key, $from_view = '')
    {
        ...
        $skinurl = base_url( VIEW_DIR . 'media/' . $skindir);
        ...
        $sfield = $sfieldchk = 'post_nickname';
        ...
        $this->Post_model->search_field_equal = array('post_id', 'post_userid' ); // 검색중 like 가 아닌 = 검색을 하는 필드
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['modify_url'] = ($can_modify && ! element('post_del', $val))
                    ? site_url('media_write/modify/'.element('post_id', $val)) . '?' . $param->output() : '';
                $result['list'][$key]['delete_url'] = ($can_delete && ! element('post_del', $val))
                    ? site_url('postact/media_delete/' . element('post_id', $val)) . '?' . $param->output() : '';
                $result['list'][$key]['reply_url'] = ($can_reply === true && ! element('post_del', $val))
                    ? site_url('media_write/reply/'.element('post_id', $val)) : '';
        ...
        $config['base_url'] = site_url('media_write/'.$brd_key). '?' . $param->replace('page');
        ...
        if ($skeyword) {
            $return['list_url'] = site_url('media_write/'.element('brd_key', $board));
            $return['search_list_url'] = site_url('media_write/'.element('brd_key', $board));
        } else {
            $return['list_url'] = site_url('media_write/'.element('brd_key', $board)) . '?' . $param->output();
        ...
        $return['write_url'] = '';
        if ($can_write === true) {
            $return['write_url'] = site_url('media_write/'.$brd_key);
            // if(strpos($brd_key,'_review' )!==false){
