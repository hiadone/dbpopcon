...
class Selfcert extends CB_Controller
{
     ...
     public function ipin()
    {
        ...
        if($this->cbconfig->item('use_selfcert_ipin') == 'kcb') {
            ...
            $this->load->view('selfcert/kcb/ipin_form', $view);
        }
    }
     ...
     public function kcb_ipin_return()
    {
        ...
        */
        ...
        $already = $this->Member_meta_model->is_already_selfcert($mem_id, element(0, $selfcertinfo));
        ...
        $this->load->view('selfcert/kcb/ipin_result', $view);
     ...
     public function phone()
    {
        ...
        if($this->cbconfig->item('use_selfcert_phone') == 'kcb') {
            ...
            **************************************************************************/
            ...
            $this->load->view('selfcert/kcb/phone_form', $view);
        }
        ...
        if($this->cbconfig->item('use_selfcert_phone') == 'kcp') {
            ...
            $this->load->view('selfcert/kcp/phone_form', $view);
        }
        ...
        if($this->cbconfig->item('use_selfcert_phone') == 'nice') {
            ...
            // $reqseq = "REQ_".config_item('encryption_key').$this->input->get('post_id',null,0);     // 요청 번호, 이는 성공/실패후에 같은 값으로 되돌려주게 되므로
                             // 업체에서 적절하게 변경하여 쓰거나, 아래와 같이 생성한다.
                             
            // 실행방법은 싱글쿼터(`) 외에도, 'exec(), system(), shell_exec()' 등등 귀사 정책에 맞게 처리하시기 바랍니다.
            $reqseq = exec($niceconfig['home_dir'].' SEQ '.$niceconfig['site_cd'].$this->input->get('post_id',null,0));
            ...
            $niceconfig['param_r1']=$this->input->get('post_id',null,0);

            $niceconfig['param_r2']=$this->input->get('msh_mem_id',null,'');

            $niceconfig['param_r3']=$this->input->get('redirecturl',null,'');
     ...
     public function kcb_phone_return()
    {
        ...
        */
        ...
        $this->load->view('selfcert/kcb/phone_result', $view);
     ...
     public function kcp_phone_return()
    {
        ...
        $this->load->view('selfcert/kcp/phone_result', $view);
        ...
        if ($xpay->TX()) {
            ...
            */
            ...
            if( "0000" == $xpay->Response_Code() ) {
                ...
                $this->load->view('selfcert/lg/phone_result', $view);
        ...
        if ($enc_data != "") {
            ...
            if ($plaindata == -1){
                ...
                if(strcmp($this->session->REQ_SEQ, $requestnumber) != 0)
                {
                    ...
                    $selfcertinfo['message'] = "세션값이 다릅니다. 올바른 경로로 접근하시기 바랍니다.";
                } else {
                    ...
                    // $msh_mem_id_cnt = $this->Media_selfcert_history_model->count_by($countwhere);

                    // if($msh_mem_id_cnt > 0 ){
                    //     $selfcertinfo['message'] = '이미 참여한 아이디 입니다.';
                    ...
                    $msh_mobileno_cnt = $this->Media_selfcert_history_model->count_by($countwhere);

                    if($msh_mobileno_cnt > 0 ){
                        $selfcertinfo['message'] = '이미 참여한 사용자 입니다.';
                        $view['view']['selfcert_result'] = 'fail';
                        // $msh_status=3;
                    }
                    ...
                    if($view['view']['selfcert_result']==='success'){
                        if(element('mobileno', $selfcertinfo)) {
                            
                            $this->session->set_userdata('mobileno', element('mobileno', $selfcertinfo));
                        }
                        ...
                        // $msh_status_1 = $this->Media_selfcert_history_model->get_one('', '', $where);

                        if(element('post_id',$msh_status_3)){                                                       

                            $view['view']['redirecturl'] = base_url('media/reward_render/selfcert_ad/'.element('msh_key', $msh_status_3));
                            $selfcertinfo['message'] = '감사합니다 이벤트 페이지로 안내합니다.';        
                        // } elseif(element('post_id',$msh_status_1)){

                        //     $view['view']['redirecturl'] = $this->input->post('param_r3', null, '').'/' . element('msh_key', $msh_status_1);
                            ...
                            $verificationcode = str_replace(
                                $vericode,
                                '',
                                password_hash($msh_id . '-' . element('mobileno', $selfcertinfo) . '-' . random_string('alnum', 10), PASSWORD_BCRYPT)
                            );
                            ...
                            $view['view']['redirecturl'] = $this->input->post('param_r3', null, '').'/' . $verificationcode;        
                        }
                    }
                }
            }
