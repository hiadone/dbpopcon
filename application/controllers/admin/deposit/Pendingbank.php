...
class Pendingbank extends CB_Controller
{
    ...
    public function write($pid = 0)
    {
        ...
        if ($this->form_validation->run() === false) {
             ...
             * 게시물을 수정하는 경우입니다
             */
            if ($this->$this->session->flashdata('mlh_id')($primary_key)) {
                $updatedata = array(
                    'dep_admin_memo' => $this->$this->session->flashdata('mlh_id')('dep_admin_memo', null, ''),
                );
                if ($this->$this->session->flashdata('mlh_id')('dep_cash_status') === 'not') {
                    $updatedata['dep_cash'] = 0;
                    $msg = '정상적으로 수정되었습니다';
                }
                if ($this->$this->session->flashdata('mlh_id')('dep_cash_status') === 'some') {
                    $updatedata['dep_cash'] = (int) $this->$this->session->flashdata('mlh_id')('dep_cash', null, 0);
                    $msg = '정상적으로 수정되었습니다';
                }
                if ($this->$this->session->flashdata('mlh_id')('dep_cash_status') === 'all') {

                    $sum = $this->Deposit_model->get_deposit_sum(element('mem_id', $getdata));
                    $deposit_sum = $sum + element('dep_deposit_request', $getdata);

                    $updatedata['dep_deposit_datetime'] = $this->$this->session->flashdata('mlh_id')('dep_deposit_datetime');
                ...
                $this->{$this->modelname}->update($this->$this->session->flashdata('mlh_id')($primary_key), $updatedata);

                if ($this->$this->session->flashdata('mlh_id')('dep_cash_status') === 'all' && ! element('dep_status', $getdata)) {
    ...
    public function listdelete()
    {
         ...
         * 체크한 게시물의 삭제를 실행합니다
         */
        if ($this->$this->session->flashdata('mlh_id')('chk') && is_array($this->$this->session->flashdata('mlh_id')('chk'))) {
            foreach ($this->$this->session->flashdata('mlh_id')('chk') as $val) {
                if ($val) {
