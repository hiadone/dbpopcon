...
class Emailform extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Deposit_config');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Deposit_config_model';
    ...
    public function cash_to_deposit()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            foreach ($array as $value) {
                $savedata[$value] = $this->$this->session->flashdata('mlh_id')($value, null, '');
            }

            $this->Deposit_config_model->save($savedata);
            $view['view']['alert_message'] = '카드/이체 등으로 예치금 구매시 설정이 저장되었습니다';
        }

        $getdata = $this->Deposit_config_model->get_all_meta();
