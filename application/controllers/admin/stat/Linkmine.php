 ...
 */
class Linkmine extends CB_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'stat/linkmine';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Media_linkmine_history','Post');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Media_linkmine_history_model';
    ...
    public function index()
    {
        ...
        $where = array();
        $where_in='';
        if ($this->input->get('mlh_status')) {
            $where['mlh_status'] = $this->input->get('mlh_status');
        }
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('mlh_name','mlh_mobileno'); // 검색이 가능한 필드
        //$this->{$this->modelname}->search_field_equal = array('mlh_name','mlh_mobileno'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('mlh_name'); // 정렬이 가능한 필드
         ...
         */
        $search_option = array('mlh_name' => '이름','mlh_mobileno' => '핸드폰 번호');
